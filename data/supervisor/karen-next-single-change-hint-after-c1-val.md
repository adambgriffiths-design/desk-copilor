...
**Delay-vs-suppress refined entry-wait gate (hop 2 — measure first).**

| Field | Value |
|-------|--------|
| **id** | `c4_shadow_quality_gated_wait` |
| **single_change** | Default FORCE_WAIT stays for `entryStatus==="WAIT"`; unlock directional L/S **only** for pre-declared **brief-delay** class; permanent-suppress stays WAIT; EXTENDED always waits |
| **vs binary c1** | Not “WAIT never blocks” |

Hypothesis (DEV only; shadow / paired asOfs; no production edit yet):

> Among one-sided-support stamps currently forced to WAIT by `entryStatus==="WAIT"`, classify each as **brief delay** (same-direction ACTIVE / actionable within a short horizon, e.g. 5/15/30m) vs **permanent session suppression**. Unlock only the delay class while keeping suppression where quality collapses.
...
- Binary removal proved volume can rise with DEV lift that **fails VAL expectancy** — need selectivity (delay vs suppress).
