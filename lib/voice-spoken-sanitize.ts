/** MNQ-style futures prices (25000–32000). */
...
/** Voice-only trim — panel/chat bubble text is unchanged. */
export function sanitizeSpokenBrief(
  text: string,
  opts?: { maxPrices?: number; levelsQuestion?: boolean }
): string {
  const maxPrices = opts?.maxPrices ?? (opts?.levelsQuestion ? 4 : 3);
  let out = String(text || "")
    .replace(/^META:.*$/gim, "")
    .replace(/\*\*/g, "")
...
export function countMnqPrices(text: string): number {
  return (String(text || "").match(MNQ_PRICE) || []).length;
}
