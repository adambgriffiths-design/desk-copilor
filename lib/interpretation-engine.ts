...
export function buildMarketInterpretation(obs: ReadonlyMarketObservation): MarketInterpretation {
  ...
  if (obs.market_structure === "bullish") {
  ...
  if (swept.length) {
    ...
    longReasons.push(`Liquidity sweep observed at ${swept.map((l) => l.label).join(", ")}`);
    shortReasons.push(`Liquidity sweep observed at ${swept.map((l) => l.label).join(", ")}`);
  }
  ...
  if (obs.displacement === "present") {
    ...
    longReasons.push("Displacement present after sweep");
    shortReasons.push("Displacement present after sweep");
  }
  ...
  } else if (obs.fvg.status === "present" && obs.displacement === "present") {
    entry_model = "Displacement + FVG retrace entry";
  } else if (obs.market_structure !== "unclear" && obs.market_structure !== "unknown") {
